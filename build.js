...
function scriptHtml() {
  ...
  var _fbApp = null, _fbAuth = null, _fbDb = null, _currentUser = null, _isPro = false;
  ...
  function checkUsage() {
    var u = getUsage();
    var remaining = Math.max(0, 3 - u.count);
    var badge = document.getElementById('usage-badge');
    if (!badge) return;
    if (_isPro) {
      var proRemaining = Math.max(0, 500 - u.count);
      badge.textContent = 'PRO ' + proRemaining + '/500';
  ...
  async function sendChat() {
    ...
    var u = getUsage();
    if (_isPro && u.count >= 500) { showPaywall(); input.value = ''; return; }
    if (!_isPro && u.count >= 3) { showPaywall(); input.value = ''; return; }
