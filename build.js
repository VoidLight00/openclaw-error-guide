...
function scriptHtml() {
  ...
  <div id="chatbot-panel" style="display:none;position:fixed;bottom:90px;right:24px;width:380px;max-height:520px;background:#1a1a2e;border:1px solid #333;border-radius:16px;box-shadow:0 8px 32px rgba(0,0,0,0.5);z-index:9999;overflow:hidden;font-family:-apple-system,BlinkMacSystemFont,sans-serif">
    ...
    <div style="padding:10px 12px;background:#16162a;border-bottom:1px solid #333">
      <select id="chat-provider" style="width:48%;padding:6px;border-radius:8px;border:1px solid #444;background:#1a1a2e;color:#ddd;font-size:12px;margin-right:2%">
        ...
        <option value="openrouter">OpenRouter (무료모델)</option>
      ...
      <input id="chat-apikey" type="password" placeholder="API Key (Groq는 생략 가능)" style="width:48%;padding:6px;border-radius:8px;border:1px solid #444;background:#1a1a2e;color:#ddd;font-size:12px">
    </div>
    <div id="chat-messages" style="height:300px;overflow-y:auto;padding:12px;display:flex;flex-direction:column;gap:8px">
      <div style="background:#2a2a4a;padding:10px 14px;border-radius:12px;color:#ccc;font-size:13px;max-width:85%">
        안녕하세요! 오류 증상을 설명해주세요.<br>예: "설치했는데 화면이 안 떠요"
      </div>
    </div>
    <div style="padding:10px 12px;border-top:1px solid #333;display:flex;gap:8px">
      <input id="chat-input" type="text" placeholder="증상이나 오류 메시지 입력..." onkeydown="if(event.key==='Enter')sendChat()" style="flex:1;padding:10px 14px;border-radius:24px;border:1px solid #444;background:#16162a;color:#ddd;font-size:13px;outline:none">
      <button onclick="sendChat()" style="padding:10px 16px;border-radius:24px;border:none;background:#6366f1;color:white;cursor:pointer;font-size:13px;font-weight:600">전송</button>
    </div>
  </div>
  ...
  async function sendChat() {
    ...
    if (!apiKey && provider !== 'groq') { alert('API Key를 입력해주세요 (Groq는 키 없이 사용 가능)'); return; }
    // Save
    ...
    input.value = '';
    box.innerHTML += '<div id="typing" style="background:#2a2a4a;padding:10px 14px;border-radius:12px;color:#888;font-size:13px">분석 중...</div>';
    ...
    try {
      ...
      document.getElementById('typing')?.remove();
      box.innerHTML += '<div style="background:#4a1a1a;padding:10px 14px;border-radius:12px;color:#f88;font-size:13px">오류: '+e.message+'</div>';
    }
    box.scrollTop = box.scrollHeight;
  }
  </script>`;
